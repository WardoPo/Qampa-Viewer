...
function Photo({ src, description = "", gridInfo = {}, onClick }) {

    let maxRoundedHeight = isNaN(Math.round(gridInfo.maxHeight)) ? "500" : Math.round(gridInfo.maxHeight)

    return (
        <div className="d-inline-block p-1" style={{ maxHeight: Math.floor(gridInfo.maxHeight), aspectRatio: gridInfo.aspect_ratio }} onClick={onClick}>
